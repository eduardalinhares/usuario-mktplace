package br.com.senai.usuariosmktplace.core.dao.postgress;
...
public class DaoPostgresUsuario implements DaoUsuario {
	...
	public void alterar(Usuario usuario) {
		...
		try {
			...
			boolean isAlteraçãoOK = ps.executeUpdate() == 1;
			...
			throw new RuntimeException("Ocorreu um erro ao alterar o usuario. Motivo: " + e.getMessage());
	...
	public Usuario buscarPor(String login) {
		...
		try {
			...
			throw new RuntimeException("Ocorreu um erro ao buscar o usuario. Motivo: " + e.getMessage());
		} finally {
			ManagerDb.getInstance().fechar(ps);
